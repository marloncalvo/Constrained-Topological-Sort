...
if [ ! -f ConstrainedTopoSort.java ]; then
	echo ""
...
if ! [ -x "$(command -v grep)" ]; then
	...
	num_spc_lines=`grep "^ " ConstrainedTopoSort.java | wc -l | awk '{$1=$1};1'`
	num_tab_lines=`grep "$(printf '^\t')" ConstrainedTopoSort.java | wc -l | awk '{$1=$1};1'`
	num_und_lines=`grep "$(printf '^[^\t ]')" ConstrainedTopoSort.java | wc -l | awk '{$1=$1};1'`
	...
	if [ "$num_spc_lines" -gt 0 ] && [ "$num_tab_lines" -gt 0 ]; then
		...
		echo ""
		echo "     grep \"^ \" ConstrainedTopoSort.java -n"
		echo "     grep \"\$(printf '^\t')\" ConstrainedTopoSort.java -n"
